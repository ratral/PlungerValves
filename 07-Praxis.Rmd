...
```{r include=FALSE}
  ...
  # Table for the plot
  df <- tibble( factor = c( "Kv", "Fl", "sigma_1", "zeta" ), 
                     y = c( kv_needed, fl, sig_1, zeta ), 
...
$$
the Required $K_v$ is `r round(kv_needed, 1)` $m^3 / h$ and the relative flow coefficient is $K_v / K_{vs}=$ `r round(kv_kvs,2)`%.
