...
The goal of this exercise is to demonstrate the verification conditions of a preset control valve. The sizing of a valve without fittings (diameter of the valve equal to the diameter of the pipes), constant flow, constant upstream, and downstream pressures will be controlled..
...
```{r include=FALSE}
  ...
  flow <- 0.250   # m3/s
  d    <- 1.00    # m
  D1   <- 1.353   # m (DN Pipe Upstream)
  D2   <- 1.353   # m (DN Pipe Downstream)
  
  # Cylinder parameter
  
  parameters <- tribble(
    ~typ,          ~kv.b,  ~kv.d,   ~kv.e,   zvs,    fls,
    "E",          -2.393,	1.389,	67.422,	  1.780,	0.617,
    "SZ-40",      -2.755, 1.979,  99.215,  15.830,  0.775,
    "SZK-30-20%", -3.893,	1.146,  61.038, 	3.000,	0.598
  )
```
