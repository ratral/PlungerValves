...
Sizing a valve without fittings, constant flow, constant upstream, and constant downstream pressures.
   ...
   - Step 5. Determine the pressure drop to use for sizing ($\Delta{P_{sizing}}).$
...
```{r sigma-Example-1, echo=FALSE, warning=FALSE, fig.cap="Sigma"}
  ...
  plot_sigma( kv.b, kv.d, kv.e, fls, cylinder) +
    geom_point( data = data_points, aes( y = y, x = x ), 
                colour = "black", size = 2) +
     geom_segment(data = arrows_points, aes(x = x0, y = y0, xend = x1, yend = y1), linetype = "dotted") +
     geom_segment(data = arrows_points, aes(x = x1, y = y1, xend = x8, yend = y8), linetype = "dotted") +
     geom_segment(data = arrows_points, aes(x = x2, y = y2, xend = x3, yend = y3), linetype = "dotted") +
     geom_segment(data = arrows_points, aes(x = x4, y = y4, xend = x5, yend = y5), linetype = "dotted") +
     geom_segment(data = arrows_points, aes(x = x6, y = y6, xend = x7, yend = y7), linetype = "dotted")
